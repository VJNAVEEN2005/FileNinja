...
export default function PdfToPng() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        const viewport = page.getViewport({ scale: 2.0 }); // High quality for PNG
      ...
      setResult({
        blob: content,
        name: file.name.replace(/\.[^/.]+$/, '') + '_images_png.zip',
        size: content.size,
        count: numPages
      });
    } catch (err) {
  ...
  return (
    <>
      <Navbar />
      <SEO title="PDF to PNG" description="Convert PDF pages to PNG with transparency in your browser." path="/pdf-to-png" />
      <ToolPageLayout
        title="PDF to PNG"
        description="Extract each page of your PDF as a high-quality PNG image with transparency."
      >
        ...
        {!result ? (
          <div className="tool-section">
            <FileDropZone 
              ...
              sublabel="Extract pages as PNG images" 
            />
          ...
          <DownloadBanner 
            ...
            info={`${result.count} pages converted successfully.`}
