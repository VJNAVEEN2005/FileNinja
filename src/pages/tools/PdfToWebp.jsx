...
export default function PdfToWebp() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        const base64Data = imgData.replace(/^data:image\/webp;base64,/, "");
      ...
      setResult({
        blob: content,
        name: file.name.replace(/\.[^/.]+$/, '') + '_images_webp.zip',
        size: content.size,
        count: numPages
      });
    } catch (err) {
      console.error(err);
      alert('Error converting PDF to WebP.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Navbar />
      <SEO title="PDF to WebP" description="Convert PDF pages to WebP for web optimization." path="/pdf-to-webp" />
      <ToolPageLayout
        title="PDF to WebP"
        description="Convert your PDF pages to optimized WebP images for faster web loading."
      >
        ...
        {!result ? (
          <div className="tool-section">
            <FileDropZone 
              ...
              sublabel="Optimize pages for web as WebP" 
            />
          ...
          <DownloadBanner 
            ...
            info={`${result.count} pages converted to WebP successfully.`}
            savedText={`ZIP archive generated — ${formatFileSize(result.size)}`}
            blob={result.blob}
